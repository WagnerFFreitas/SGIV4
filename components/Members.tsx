...
const MemberIDCard: React.FC<{ member: Member }> = ({ member }) => (
  <div className="flex flex-row items-start justify-center gap-0 print:mb-0 mb-4 no-break id-card-element bg-white" style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
    {/* Frente */}
    <div className="w-[86mm] h-[54mm] bg-gradient-to-br from-slate-900 to-indigo-950 rounded-l-lg shadow-md relative overflow-hidden flex flex-col items-center p-3 text-white border border-slate-800 border-r-0 shrink-0">
      ...
      <div className="flex items-center gap-2 mb-2 relative z-10 w-full">
        ...
        <div className="text-left">
          <p className="text-[10px] font-black uppercase tracking-widest leading-none">ADJPA</p>
          <p className="text-[5px] font-bold text-indigo-400 uppercase tracking-tighter">Assembleia de Deus</p>
        </div>
        <div className="ml-auto">
           <span className="text-[5px] font-black px-1 py-0.5 bg-indigo-600 rounded text-white uppercase">Membro</span>
        </div>
      </div>
      <div className="flex w-full gap-3 relative z-10 mt-1">
        <div className="w-16 h-20 rounded-md border-2 border-white/20 overflow-hidden shadow-xl bg-slate-800">
          <img 
            src={member.avatar} 
        ...
        <div className="flex-1 flex flex-col justify-center">
           <h4 className="text-[10px] font-black uppercase leading-tight mb-0.5 text-indigo-100">{member.name}</h4>
           <p className="text-[7px] font-black text-indigo-400 uppercase tracking-widest mb-1.5">{member.ecclesiasticalPosition || 'MEMBRO ATIVO'}</p>
           <div className="mt-auto space-y-0.5">
              <p className="text-[4px] font-black text-slate-400 uppercase leading-none">Congregação</p>
              <p className="text-[6px] font-bold text-white uppercase">SEDE MUNDIAL</p>
           </div>
        </div>
      </div>
    ...
    {/* Verso */}
    <div className="w-[86mm] h-[54mm] bg-white rounded-r-lg shadow-md relative overflow-hidden flex p-3 border border-slate-200 border-l-0 shrink-0">
      <div className="flex-1 flex flex-col justify-between text-slate-800">
        <div className="space-y-1.5">
          <p className="text-[8px] font-black border-b border-slate-100 pb-1 mb-1 uppercase tracking-tighter text-slate-900">Credencial de Identificação</p>
          <div className="grid grid-cols-2 gap-x-2 gap-y-1.5">
            <div>
              <p className="text-[4px] font-black text-slate-700 uppercase">CPF</p>
              <p className="text-[7px] font-bold text-slate-900">{member.cpf || '---.---.---'}</p>
            </div>
            <div>
              <p className="text-[4px] font-black text-slate-700 uppercase">Data Nasc.</p>
              <p className="text-[7px] font-bold text-slate-900">{member.birthDate ? new Date(member.birthDate + 'T12:00:00').toLocaleDateString('pt-BR') : '--/--/----'}</p>
            </div>
            <div>
              <p className="text-[4px] font-black text-slate-700 uppercase">Data Batismo</p>
              <p className="text-[7px] font-bold text-slate-900">{member.baptismDate ? new Date(member.baptismDate + 'T12:00:00').toLocaleDateString('pt-BR') : 'Não Informado'}</p>
            </div>
            <div>
              <p className="text-[4px] font-black text-slate-700 uppercase">Validade</p>
              <p className="text-[7px] font-black text-rose-700">31/12/2025</p>
            </div>
          </div>
        </div>
        <div className="mt-2 pt-2 border-t border-dashed border-slate-300 text-center">
          <p className="text-[5px] text-slate-800 uppercase font-black tracking-widest">Assinatura do Pastor Presidente</p>
        </div>
      </div>
      <div className="w-16 flex flex-col items-center justify-center gap-2 border-l border-slate-50 pl-3">
        <div className="p-1 bg-white rounded border border-slate-100 shadow-sm">
          <QrCode size={40} className="text-slate-900"/>
        </div>
        <div className="text-center">
          <p className="text-[4px] font-black text-indigo-700 uppercase leading-tight">VALIDAÇÃO<br/>DIGITAL</p>
        </div>
      </div>
    </div>
  </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h1 className="text-xl font-bold text-slate-900 leading-tight">Gestão de Membresia</h1>
          <p className="text-slate-500 font-medium text-sm">Cuidado pastoral e administrativo em tempo real.</p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          <button 
            onClick={handleOpenBatchCards}
            className="flex items-center justify-center gap-2 px-6 py-2.5 bg-slate-200 text-slate-600 rounded-lg font-bold text-xs uppercase transition-all hover:bg-slate-300"
          >
            <Printer size={16} /> Imprimir Carteirinhas
          </button>
          
          <button onClick={() => openModal()} className="flex items-center justify-center gap-2 px-6 py-2.5 bg-slate-900 text-white rounded-lg font-bold text-xs uppercase shadow-lg transition-all hover:bg-slate-800">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Membros Ativos', val: stats.active, icon: <User size={18} className="text-indigo-600"/> },
          { label: 'Líderes Ativos', val: stats.leaders, icon: <Star size={18} className="text-indigo-600"/> },
          { label: 'Visitantes / Transição', val: stats.visitors, icon: <TrendingUp size={18} className="text-indigo-600"/> },
          { label: 'Total Registrados', val: members.length, icon: <Users size={18} className="text-indigo-600"/> },
        ].map((s, i) => (
      ...
      <div className="bg-white p-2 rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row gap-2">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="Buscar por nome, e-mail ou CPF..." className="w-full pl-10 pr-4 py-2.5 bg-transparent outline-none text-sm text-slate-900 font-medium" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
          ...
          <table className="w-full text-left min-w-[1000px]">
            <thead className="bg-slate-50/30 text-[11px] text-slate-500 font-bold uppercase tracking-wider border-b border-slate-100">
              <tr>
                ...
                <th className="px-8 py-4">Cargo / Ministério</th>
                <th className="px-8 py-4">Vínculos</th>
              ...
              {filteredMembers.map((member) => (
                <tr key={member.id} className={`hover:bg-slate-50/50 transition-all ${selectedMemberIds.includes(member.id) ? 'bg-indigo-50/20' : ''}`}>
                  <td className="px-6 py-4 text-center">
                    <div onClick={() => toggleSelectMember(member.id)} className="cursor-pointer">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <img 
                          ...
                          onError={(e) => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${member.name}&background=1e293b&color=fff`; }}
                        />
                        {member.isTithable && <div className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-emerald-500 border-2 border-white rounded-full flex items-center justify-center"><DollarSign size={8} className="text-white"/></div>}
                      </div>
                      <div>
                        <p className="font-bold text-slate-900 text-sm leading-none mb-1">{member.name}</p>
                        <p className="text-[10px] text-slate-500 font-medium uppercase tracking-tighter">{member.phone}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-8 py-4">
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 flex items-center justify-center font-black text-indigo-800 text-[10px]">F</div>
                      <span className="text-[10px] font-black text-slate-700 uppercase tracking-tighter">Ver Vínculos</span>
                    </div>
                  </td>
                  <td className="px-8 py-4">
                     <span className={`px-2.5 py-1 rounded text-[10px] font-black uppercase ${member.isTithable ? 'text-emerald-700 bg-emerald-50' : 'text-slate-600 bg-slate-100'}`}>
                      {member.isTithable ? 'Dizimista' : 'Visitante'}
                     </span>
                  </td>
                  <td className="px-8 py-4">
                    <span className="px-2.5 py-1 rounded text-[10px] font-black uppercase text-emerald-700 bg-emerald-50">
                      Regular
                    </span>
                  </td>
                  <td className="px-8 py-4 text-right">
                    <div className="flex justify-end gap-3">
                      <button onClick={() => { setEditingMember(member); setIsIDCardOpen(true); }} className="text-slate-400 hover:text-indigo-600 transition-colors" title="Imprimir Carteirinha">
                        <QrCode size={18} />
                      </button>
                      <button onClick={() => openModal(member)} className="text-slate-400 hover:text-indigo-600 transition-colors" title="Editar Cadastro">
                        <Edit2 size={18} />
                      </button>
                    </div>
                  </td>
      ...
      {/* MODAL DE IMPRESSÃO */}
      {isIDCardOpen && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl animate-in fade-in duration-300 no-print">
           <div className="bg-white rounded-[2rem] w-full max-w-5xl shadow-2xl relative flex flex-col max-h-[90vh]">
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-10 bg-slate-200/50 custom-scrollbar" id="printable-area-multi">
                {chunkedMembers.map((group, pageIndex) => (
                  <div key={pageIndex} className="a4-page-mimic bg-white mx-auto shadow-lg border border-slate-200 flex flex-col items-center p-[10mm] mb-12 last:mb-0 relative" style={{ width: '210mm', height: '297mm', minHeight: '297mm' }}>
