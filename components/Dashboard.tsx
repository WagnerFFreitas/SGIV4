  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-black text-slate-900 leading-tight">Olá, {user.name.split(' ')[0]}</h1>
        ...
        <div className="flex gap-2">
          <button className="bg-indigo-600 text-white px-4 py-2.5 rounded-xl font-bold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-100 flex items-center gap-2 text-xs uppercase">
            <ShieldCheck size={16} /> Auditoria Rápida
          </button>
        </div>
      </div>
        ...
        {[
          ...
          <div key={i} className="bg-white p-6 rounded-[1.5rem] border border-slate-100 shadow-sm hover:shadow-md transition-all group">
            <div className="flex justify-between items-start mb-4">
              <div className={`p-3 rounded-xl bg-indigo-50 text-indigo-600 group-hover:scale-110 transition-transform shadow-sm`}>
                {stat.icon}
              </div>
      ...
      {cnhAlerts.length > 0 && (
        <div className="bg-rose-50 border border-rose-100 rounded-[2rem] p-6 animate-in slide-in-from-top-4 duration-500">
           <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <h3 className="text-sm font-black text-rose-900 uppercase tracking-tight">Alertas Críticos de Documentação</h3>
                    <p className="text-[10px] font-bold text-rose-600 uppercase mt-0.5">Vencimentos de CNH Próximos</p>
                 </div>
              </div>
              <span className="bg-rose-600 text-white text-[10px] font-black px-2.5 py-1 rounded-full">{cnhAlerts.length} PENDÊNCIAS</span>
           </div>
           
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {cnhAlerts.map(alert => (
                 <div key={alert.id} className="bg-white p-4 rounded-2xl border border-rose-100 shadow-sm flex items-center justify-between group hover:shadow-md transition-all">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          <p className="text-xs font-black text-slate-900 leading-tight">{alert.employeeName.split(' ')[0]} {alert.employeeName.split(' ').slice(-1)}</p>
                          <p className="text-[9px] font-bold text-slate-500 uppercase">Venc: {new Date(alert.cnh_vencimento! + 'T12:00:00').toLocaleDateString('pt-BR')}</p>
                       </div>
                    </div>
                    <div className="text-right">
                       <p className={`text-[10px] font-black uppercase ${alert.isExpired ? 'text-rose-600' : 'text-amber-600'}`}>
                          {alert.isExpired ? 'VENCIDA' : `Faltam ${alert.daysToExpiry} dias`}
                       </p>
                       <AlertTriangle size={14} className={`ml-auto mt-1 ${alert.isExpired ? 'text-rose-600' : 'text-amber-500'} animate-pulse`} />
                    </div>
                 </div>
              ))}
           </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col flex-1">
            <div className="p-6 border-b border-slate-50 bg-indigo-50/30 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="p-2 bg-indigo-600 text-white rounded-lg shadow-lg">
                  <Cake size={18} />
                </div>
                <h3 className="font-black text-slate-900 text-sm">Aniversariantes do Mês</h3>
              </div>
            ...
            <div className="flex-1 overflow-y-auto max-h-[380px] p-6 space-y-4 custom-scrollbar">
              {anniversaries.thisMonth.length === 0 ? (
                ...
                anniversaries.thisMonth.map((person, idx) => {
                  ...
                  return (
                    <div key={idx} className={`p-4 rounded-2xl border transition-all flex items-center justify-between group ${personIsToday ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl shadow-indigo-100' : 'bg-slate-50 border-slate-100 hover:bg-white hover:shadow-md hover:border-indigo-200'}`}>
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          <img src={person.avatar} className="w-10 h-10 rounded-xl object-cover border-2 border-white shadow-sm" alt="" />
                          {personIsToday && (
                             <div className="absolute -top-1 -right-1 w-3 h-3 bg-emerald-400 border-2 border-indigo-600 rounded-full animate-ping" />
                          )}
                        </div>
                        <div>
                          <p className={`text-sm font-black leading-tight ${personIsToday ? 'text-white' : 'text-slate-900'}`}>{person.name.split(' ')[0]} {person.name.split(' ').slice(-1)}</p>
                          <p className={`text-[10px] font-bold uppercase tracking-tighter ${personIsToday ? 'text-indigo-200' : 'text-slate-600'}`}>{person.type} • Dia {bDate.getDate()}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-end">
                        {personIsToday ? (
                          ...
                          <button className="p-2 text-slate-500 hover:text-indigo-600 group-hover:bg-indigo-50 rounded-lg transition-all">
                             <Gift size={16}/>
                          </button>
                        )}
                      </div>
            ...
            <button className="m-6 mt-0 p-4 border-2 border-dashed border-slate-200 rounded-2xl text-[10px] font-black text-slate-600 uppercase tracking-widest hover:border-indigo-300 hover:text-indigo-600 transition-all flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 bg-slate-900 text-white p-8 rounded-[2rem] shadow-2xl relative overflow-hidden flex flex-col h-full">
          <div className="absolute top-0 right-0 p-8 opacity-5">
            <Sparkles size={120} />
          </div>
          <div className="relative z-10 flex-1">
            <div className="flex items-center gap-2 mb-8">
              <div className="bg-indigo-600 p-2 rounded-xl shadow-lg shadow-indigo-900">
                <Sparkles size={20} className="text-white" />
              </div>
              <h3 className="font-black text-lg">IA Eclesiástica</h3>
            </div>
            
            <div className="space-y-6">
              <div className="bg-white/5 p-5 rounded-2xl border border-white/5 backdrop-blur-sm">
                <p className="text-sm text-indigo-100 leading-relaxed italic font-medium">
                  "{insights}"
                </p>
              </div>
          ...
          <button className="mt-10 w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-sm hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-950 z-10">
            Gerar Relatório IA
          </button>
        </div>
